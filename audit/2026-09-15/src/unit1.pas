...
function CreateCalcBtn(AParent: TWinControl; const ACaption: string;
  AClick: TNotifyEvent; const ATag: Integer = 0): TCalcButton;
begin
...
var
  RightEdge, ModeAvailW, ModeBtnW: Integer;
begin
  if not Assigned(PanelTopBar) then Exit;
  RightEdge := PanelTopBar.ClientWidth - 8;

  // 右侧辅助按钮靠右
  BtnTheme.SetBounds(RightEdge - 36, 6, 36, 34);
  BtnTopMost.SetBounds(RightEdge - 76, 6, 36, 34);
  BtnAngle.SetBounds(RightEdge - 124, 6, 44, 34);

  // 左侧 5 个模式切换按钮自适应平分剩余空间
  ModeAvailW := (RightEdge - 132) - 8;
  ModeBtnW := Min(110, Max(56, (ModeAvailW - 4 * 6) div 5));

  BtnModeAlg.SetBounds(8 + 0 * (ModeBtnW + 6), 6, ModeBtnW, 34);
  BtnModeRPN.SetBounds(8 + 1 * (ModeBtnW + 6), 6, ModeBtnW, 34);
  BtnModeProg.SetBounds(8 + 2 * (ModeBtnW + 6), 6, ModeBtnW, 34);
  BtnModeConv.SetBounds(8 + 3 * (ModeBtnW + 6), 6, ModeBtnW, 34);
  BtnModeTape.SetBounds(8 + 4 * (ModeBtnW + 6), 6, ModeBtnW, 34);
end;
...
begin
  ...
  LabelExpr.SetBounds(16, 6, DispW, 20);
  LabelMem.SetBounds(16, 6, 40, 20);
...
begin
  ...
  FScientificRows[0][0] := CreateCalcBtn(PanelScientific, '(', @OnAlgParenOpenClick);
  FScientificRows[0][1] := CreateCalcBtn(PanelScientific, ')', @OnAlgParenCloseClick);
  FScientificRows[0][2] := CreateCalcBtn(PanelScientific, 'MC', @OnAlgMemoryClick, 1);
  FScientificRows[0][3] := CreateCalcBtn(PanelScientific, 'MR', @OnAlgMemoryClick, 2);
  FScientificRows[0][4] := CreateCalcBtn(PanelScientific, 'MS', @OnAlgMemoryClick, 3);
  FScientificRows[0][5] := CreateCalcBtn(PanelScientific, 'M+', @OnAlgMemoryClick, 4);
  FScientificRows[0][6] := CreateCalcBtn(PanelScientific, 'M-', @OnAlgMemoryClick, 5);

  // 第 1 行：2nd、三角函数、常数与 AC
  SetLength(FScientificRows[1], 7);
  Btn2nd := CreateCalcBtn(PanelScientific, '2nd', @OnAlg2ndClick);
  FScientificRows[1][0] := Btn2nd;
  BtnSin := CreateCalcBtn(PanelScientific, 'sin', @OnAlgUnaryClick);
  FScientificRows[1][1] := BtnSin;
  BtnCos := CreateCalcBtn(PanelScientific, 'cos', @OnAlgUnaryClick);
  FScientificRows[1][2] := BtnCos;
  BtnTan := CreateCalcBtn(PanelScientific, 'tan', @OnAlgUnaryClick);
  FScientificRows[1][3] := BtnTan;
  FScientificRows[1][4] := CreateCalcBtn(PanelScientific, 'π', @OnAlgUnaryClick);
  FScientificRows[1][5] := CreateCalcBtn(PanelScientific, 'e', @OnAlgUnaryClick);
  FScientificRows[1][6] := CreateCalcBtn(PanelScientific, 'AC', @OnAlgClearClick);

  // 第 2 行：幂次、对数与除法
  SetLength(FScientificRows[2], 7);
  BtnXSq := CreateCalcBtn(PanelScientific, 'x²', @OnAlgUnaryClick);
  FScientificRows[2][0] := BtnXSq;
  FScientificRows[2][1] := CreateCalcBtn(PanelScientific, 'xʸ', @OnAlgOpClick);
  BtnSqrt := CreateCalcBtn(PanelScientific, '√x', @OnAlgUnaryClick);
  FScientificRows[2][2] := BtnSqrt;
  FScientificRows[2][3] := CreateCalcBtn(PanelScientific, '1/x', @OnAlgUnaryClick);
  FScientificRows[2][4] := CreateCalcBtn(PanelScientific, 'log', @OnAlgUnaryClick);
  FScientificRows[2][5] := CreateCalcBtn(PanelScientific, 'ln', @OnAlgUnaryClick);
  FScientificRows[2][6] := CreateCalcBtn(PanelScientific, '÷', @OnAlgOpClick);

  // 第 3 行：双曲函数与数字 7, 8, 9, 乘法
  SetLength(FScientificRows[3], 7);
  BtnSinh := CreateCalcBtn(PanelScientific, 'sinh', @OnAlgUnaryClick);
  FScientificRows[3][0] := BtnSinh;
  BtnCosh := CreateCalcBtn(PanelScientific, 'cosh', @OnAlgUnaryClick);
  FScientificRows[3][1] := BtnCosh;
  BtnTanh := CreateCalcBtn(PanelScientific, 'tanh', @OnAlgUnaryClick);
  FScientificRows[3][2] := BtnTanh;
  FScientificRows[3][3] := CreateCalcBtn(PanelScientific, '7', @OnAlgNumClick);
  FScientificRows[3][4] := CreateCalcBtn(PanelScientific, '8', @OnAlgNumClick);
  FScientificRows[3][5] := CreateCalcBtn(PanelScientific, '9', @OnAlgNumClick);
  FScientificRows[3][6] := CreateCalcBtn(PanelScientific, '×', @OnAlgOpClick);

  // 第 4 行：阶乘、绝对值、立方与数字 4, 5, 6, 减法
  SetLength(FScientificRows[4], 7);
  FScientificRows[4][0] := CreateCalcBtn(PanelScientific, 'n!', @OnAlgUnaryClick);
  FScientificRows[4][1] := CreateCalcBtn(PanelScientific, '|x|', @OnAlgUnaryClick);
  BtnCube := CreateCalcBtn(PanelScientific, 'x³', @OnAlgUnaryClick);
  FScientificRows[4][2] := BtnCube;
  FScientificRows[4][3] := CreateCalcBtn(PanelScientific, '4', @OnAlgNumClick);
  FScientificRows[4][4] := CreateCalcBtn(PanelScientific, '5', @OnAlgNumClick);
  FScientificRows[4][5] := CreateCalcBtn(PanelScientific, '6', @OnAlgNumClick);
  FScientificRows[4][6] := CreateCalcBtn(PanelScientific, '-', @OnAlgOpClick);

  // 第 5 行：排列组合、MOD 与数字 1, 2, 3, 加法
  SetLength(FScientificRows[5], 7);
  FScientificRows[5][0] := CreateCalcBtn(PanelScientific, 'nPr', @OnAlgOpClick);
  FScientificRows[5][1] := CreateCalcBtn(PanelScientific, 'nCr', @OnAlgOpClick);
  FScientificRows[5][2] := CreateCalcBtn(PanelScientific, 'MOD', @OnAlgOpClick);
  FScientificRows[5][3] := CreateCalcBtn(PanelScientific, '1', @OnAlgNumClick);
  FScientificRows[5][4] := CreateCalcBtn(PanelScientific, '2', @OnAlgNumClick);
  FScientificRows[5][5] := CreateCalcBtn(PanelScientific, '3', @OnAlgNumClick);
  FScientificRows[5][6] := CreateCalcBtn(PanelScientific, '+', @OnAlgOpClick);

  // 第 6 行：正负、百分比、退格、0、点、等号 (等号跨 2 列)
  SetLength(FScientificRows[6], 6);
  FScientificRows[6][0] := CreateCalcBtn(PanelScientific, '+/-', @OnAlgSignClick);
  FScientificRows[6][1] := CreateCalcBtn(PanelScientific, '%', @OnAlgPercentClick);
  FScientificRows[6][2] := CreateCalcBtn(PanelScientific, '⌫', @OnAlgBackClick);
  FScientificRows[6][3] := CreateCalcBtn(PanelScientific, '0', @OnAlgNumClick);
  FScientificRows[6][4] := CreateCalcBtn(PanelScientific, '.', @OnAlgDotClick);
  Btn := CreateCalcBtn(PanelScientific, '=', @OnAlgEqualsClick);
...
begin
  ...
  ColW := (AvailW - 245) div 2;
  BtnWordSize.SetBounds(250, 8, Max(110, ColW), 32);
  BtnSigned.SetBounds(250 + Max(110, ColW) + 6, 8, Max(80, AvailW - (250 + Max(110, ColW) + 6) + 10), 32);
...
var
  ...
  procedure StyleButton(B: TCalcButton; const IsNum, IsOp, IsEq: Boolean);
  begin
    if not Assigned(B) then Exit;
    B.Font.Style := [fsBold];
    if IsEq then
      B.SetColors(P.EqualBtnBg, P.EqualBtnText, P.BorderColor, P.EqualBtnHover, P.EqualBtnPress)
    else if IsOp then
      B.SetColors(P.OpBtnBg, P.OpBtnText, P.BorderColor, P.OpBtnHover, P.OpBtnPress)
    else if IsNum then
      B.SetColors(P.NumBtnBg, P.NumBtnText, P.BorderColor, P.NumBtnHover, P.NumBtnPress)
    else
      B.SetColors(P.FuncBtnBg, P.FuncBtnText, P.BorderColor, P.FuncBtnHover, P.FuncBtnPress);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (TCalcButton(Ctrl).Caption = '=') then
          StyleButton(TCalcButton(Ctrl), False, False, True)
        else if (Length(TCalcButton(Ctrl).Caption) = 1) and (TCalcButton(Ctrl).Caption[1] in ['0'..'9']) then
          StyleButton(TCalcButton(Ctrl), True, False, False)
        else if (TCalcButton(Ctrl).Caption = '+') or (TCalcButton(Ctrl).Caption = '-') or
                (TCalcButton(Ctrl).Caption = '×') or (TCalcButton(Ctrl).Caption = '÷') or
                (TCalcButton(Ctrl).Caption = 'xʸ') or (TCalcButton(Ctrl).Caption = 'MOD') or
                (TCalcButton(Ctrl).Caption = 'nPr') or (TCalcButton(Ctrl).Caption = 'nCr') or
                (TCalcButton(Ctrl).Caption = 'AND') or (TCalcButton(Ctrl).Caption = 'OR') or
                (TCalcButton(Ctrl).Caption = 'XOR') or (TCalcButton(Ctrl).Caption = 'ENTER') or
                (TCalcButton(Ctrl).Caption = 'NAND') or (TCalcButton(Ctrl).Caption = 'NOR') or
                (TCalcButton(Ctrl).Caption = 'XNOR') or (TCalcButton(Ctrl).Caption = 'Lsh') or
                (TCalcButton(Ctrl).Caption = 'Rsh') or (TCalcButton(Ctrl).Caption = 'RoL') or
                (TCalcButton(Ctrl).Caption = 'RoR') then
          StyleButton(TCalcButton(Ctrl), False, True, False)
        else
          StyleButton(TCalcButton(Ctrl), False, False, False);
      end
...
var
  ResStr, ExprBackup: string;
begin
  ExprBackup := FAlgEngine.ExpressionStr;
  if ExprBackup = '' then
    ExprBackup := FAlgEngine.CurrentInput;

  if FAlgEngine.ExecuteEquals(ResStr) then
  begin
    AppendToTape(ExprBackup, ResStr);
  end;
  UpdateDisplays;
end;
