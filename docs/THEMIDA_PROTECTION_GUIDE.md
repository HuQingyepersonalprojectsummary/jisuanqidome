# Themida 最高加密系数保护与终端打包操作指南

本指南详细说明如何使用 **Themida 3.2.6.0**，为 **PCalc Advanced Calculator（PCalc 高级多功能科学计算器）** 配置**最高加密防护系数**，并通过终端自动化脚本一键生成加壳后的 Windows 安装包应用（Setup EXE）。

> [!TIP]
> 如果您的 Themida 安装在特定私有自定义目录，请在本地私密配置文件 `build.secrets.local.json` 中配置实际路径。公共文档与脚本中统一使用 `<THEMIDA_INSTALL_DIR>` 占位符指代，从根源防止私密路径泄漏。

---

## 一、Themida 保护机制与原生应用特性说明

1. **原生 Native x64 PE 加壳与防护**：
   - 本计算器由 Free Pascal (FPC 3.2.2) 原生编译为 Windows 64 位机器码可执行文件（`project1.exe` / `PCalc.exe`）。
   - 与依赖虚拟环境的 .NET / Python 应用不同，原生 PE 能够**100% 深度兼容 Themida 的 SecureEngine 内核**，可无缝开启最底层的 CPU 指令级虚拟化重构、反调试与反内存转储防护。
2. **终端自动化的工作方式**：
   - Themida 保护引擎使用 `.tmd` 专用工程文件保存多架构虚拟机架构、反调试钩子、加密密钥种子等参数。
   - 在首次配置并保存 `PCalc_HighSecurity.tmd` 后，终端打包脚本 `Package-ProtectedInstaller.ps1` 会全自动通过命令行 `/protect ... /q` 静默调用加壳，无需人工再次打开界面。
...
- **Input Filename**：选择项目中的计算器可执行文件，例如：
  `<PROJECT_ROOT>\releases\PCalc_Windows_Advanced_Calculator.exe` 或 `<PROJECT_ROOT>\project1.exe`
- **Output Filename**：可设置为相同路径或临时路径（例如 `PCalc_Protected.exe`）。
  > 注：终端打包脚本在运行时会自动通过 `/inputfile` 和 `/outputfile` 动态覆盖此路径，因此此处配置仅作为工程底模。

### 2. Protection Options（保护选项 —— 全部拉满）
进入左侧 **Protection Options** 面板：
- **Anti-Debugger（反调试保护）**：
  - [x] **Enable Anti-Debugger Protection**（启用多层反调试引擎）
  - [x] **Detect Kernel-Mode Debuggers**（检测 WinDbg、IDA Pro 内核级调试器）
...
- 将工程文件保存为项目根目录：
  ```text
  <PROJECT_ROOT>\PCalc_HighSecurity.tmd
  ```
...
完成上述一次性工程配置后，在终端运行以下 PowerShell 命令即可全自动完成加壳与制作安装包：
...
- **安装包程序**：`artifacts/PCalc-1.0.0-win-x64-Setup.exe`
- **校验和文件**：`artifacts/PCalc-1.0.0-win-x64-Setup.exe.sha256`

### 安装包核心特性：
1. **纯单文件安装向导**：基于 Inno Setup 6 现代化界面，自动适配 Windows 10/11 暗色与浅色高分屏 DPI。
2. **桌面与开始菜单快捷方式**：自动生成高清计算器图标，支持启动运行选项。
