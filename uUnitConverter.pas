...
{==============================================================================
  PCalc 高级科学与工程计算器 - 单位换算与物理常数数据字典 (uUnitConverter.pas)
  ------------------------------------------------------------------------------
  本单元为工程换算提供高精度的实时双向转换引擎与物理/数学常数库：
  1. 十大工程与日常生活核心单位范畴：
     - 长度 (Length): 米、千米、厘米、毫米、微米、纳米、英寸、英尺、码、英里、海里
     - 面积 (Area): 平方米、平方千米、公顷、市亩、平方厘米、平方英尺、平方码、英亩
     - 体积 (Volume): 升、毫升、立方米、美制加仑、美制品脱、液体盎司、立方英尺
     - 质量 (Mass): 千克、克、毫克、公吨、市斤、磅、盎司、克拉
     - 温度 (Temperature): 摄氏度 (°C)、华氏度 (°F)、开尔文 (K) [非线性仿射变换]
     - 速度 (Speed): 米/秒、千米/时、英里/时、节、马赫
     - 时间 (Time): 秒、毫秒、微秒、分钟、小时、天、星期、年
     - 数据 (Data): 字节、比特、KB、MB、GB、TB、PB (以 1024 二进制字节为基准)
     - 压力 (Pressure): 帕斯卡、千帕、标准大气压、巴、毫米汞柱、磅/平方英寸 (psi)
     - 能量 (Energy): 焦耳、千焦、卡路里、千卡、千瓦·时 (度)、瓦·时、电子伏特 (eV)
  2. 16 个高精度国际物理与数学常数库：
     - 真空光速 c、普朗克常数 h、约化普朗克常数 ħ、万有引力常数 G、重力加速度 g、
       玻尔兹曼常数 k、阿伏伽德罗常数 N_A、基本电荷 e、电子质量 m_e、质子质量 m_p、
       气体常数 R、介电常数 ε_0、磁导率 μ_0、圆周率 π、自然底数 e、黄金分割比 φ
     - 支持在 UI 中一键选中直接代入计算引擎参与运算
==============================================================================}
...
type
  ...
  { 单个度量衡单位条目 }
  TUnitItem = record
    Name: string;         { 单位本地化名称与符号说明 }
    FactorToBase: Double; { 换算至该大类国际标准基准单位的乘数因子 }
    Offset: Double;       { 零点偏移量 (专用于温度换算) }
  end;

  { 物理/数学常数条目 }
  TConstantItem = record
    Symbol: string;       { 常用科学符号 (如 'c', 'h', 'π') }
    Name: string;         { 中英文常数全称 }
    Value: Double;        { 高精度双精度浮点数值 }
    UnitStr: string;      { 国际标准量纲单位符号 }
  end;
...
{ 核心换算求值函数：将指定分类中 FromIdx 单位的数值换算至 ToIdx 单位 }
...
begin
  ...
  Result := BaseVal / Arr[ToIdx].FactorToBase;
end;
